...
const WeeklyView: React.FC<WeeklyViewProps> = ({
  ...
  const loadCalendarData = async () => {
    ...
    try {
      // Get the week start date for the current date
      const weekStartDate = scheduleService.getViewStartDate(currentDate, CalendarViewType.Week);
      
      const request: ScheduleRequestDto = {
        startDate: scheduleService.formatDateForApi(weekStartDate),
  ...
  const handleViewTypeChange = (viewType: CalendarViewType) => {
    // For weekly view, we don't change the view type, but we could trigger navigation
