...
const DayDetailsModal: React.FC<DayDetailsModalProps> = ({
  ...
  const dayStats = useMemo((): DayStats | null => {
    ...
    employees.forEach(employee => {
      ...
      // Count by team
      if (employee.team) {
        stats.employeesByTeam[employee.team] =
          (stats.employeesByTeam[employee.team] || 0) + 1;
      }
      ...
      // Calculate capacity (2 slots per day = 100% capacity)
      const capacity = 2; // Morning + Afternoon
      stats.totalCapacity += capacity;

      // Count tasks and analyze capacity
      const allTasks: AssignmentTaskDto[] = [
        ...(dayAssignment.morningSlot?.tasks || []),
        ...(dayAssignment.afternoonSlot?.tasks || [])
      ];

      stats.totalTasks += allTasks.length;
      stats.usedCapacity += allTasks.length;
        ...
        >
          ...
          <div style={{
            ...
            <div>
              ...
              <div style={{ backgroundColor: '#f9fafb', padding: '16px', borderRadius: '6px' }}>
                {Object.entries(dayStats.employeesByTeam).map(([team, count]) => (
                  <div key={team} style={{
                    display: 'flex',
                    justifyContent: 'space-between',
                    marginBottom: '8px'
